...
from bookInputOutput import get_files_in_folder, CSV_COMMA, getObject
...
def makecluster_hier(bookarray, books_labels):
...
def makecluster_kmean(bookarray, books_labels):
    kmeans_tests = {}
    for k in range(2,15):
...
def makecluster(signature, withdegrees):
    books = []
    books_labels=[]
    ...
    #makecluster_hier(bookarray, books_labels)
    makecluster_kmean(bookarray, books_labels)
...
def export_signature_table(signature):
    ...
    for book in signature.keys():
        ...
        csv += str(signature[book]["Clustering"]) + CSV_COMMA + str(signature[book]["Degree"]) + "\n"
