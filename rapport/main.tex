...
\usepackage{multibib}
\newcites{movies}{Movie script analyzed}
\nocitemovies{*}
\bibliographystylemovies{apalike}
\usepackage{url}
\usepackage{lscape}
...
% >>> autorisation
 \includepdf[pages={1}]{memoire-autorisation-1}
...
\textbf{Keywords}:\textit{Social network analysis, Natural Language Processing (NLP), Network Theory, Alias Resolution, Litterary Analysis, Movie Script Analysis}
\end{abstract}
...
\hline
Novel & Proposed method & \cite{original} & \cite{character_meta}\\
\hline
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{pictures/Context_graph.png}
...
\bibliography{biblio}
\bibliographymovies{movies}
\input{biblio_books.tex}
