...
Before extracting or analyzing any social networks, let's introduce the basic concept related to network. We should note that the word network and the word graph have the same meaning, the choice of of these words usually depends on the application. For ``instance social'' network are never called  ``social graph'' but it would have the same meaning. \citep{network_theory}
\section{Network Theory: Basics}
...
Network vs graph : Same ? Network can evoluate with time ? Presence of attribute in network ? Network transport things while graphs have inherent property ?

Network theory is the study of networks. A  network or a graph are defined mathematically as a pair of set $G = (V, E)$ such that $E  \subseteq [V ]^2$. Concretely it means that element from the second set should be tuple of 2 element from the first set.
Graph are usually represented with points symbolizing element from the first set linked by lines representing element from the second set. You can see an example of this representation on figure X. In network theory, element from the first set are called ``nodes'' and element from the second set are called ``edges''. If for any pair $XY \in V$,  the link $(X,Y) = (Y,X)$ then the graph is called undirected. This is the most common type of graph. There exist also directed graphs whose edges are directed from a sender to a receiver.
...
The social network is constructed using the identification of characters in dialogs in section \ref{identification}.
